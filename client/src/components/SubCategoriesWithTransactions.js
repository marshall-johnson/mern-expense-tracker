...
const ExpenseList = ({ name, category }) => {
  ...
  return (
    <div className="bg-white border rounded-lg shadow-md m-4 p-6">
      <h2 className="text-2xl font-bold text-indigo-600 mb-4">{name}</h2>
      <Accordion>
        {data.map((sub, idx) => (
          <Accordion.Item
                ...
                {sub.budget && (
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full">
                    ...
                    >
                      ...
                      {(
                        sub.budget -
                        sub.transactions.reduce((sum, tx) => sum + tx.amount, 0)
                      ).toFixed(2)}
                    </span>
                  </div>
                )}
